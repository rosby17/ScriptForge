...
// ── SpeedScript config ─────────────────────────────────────
const SS_TONES = [
  { id: 'authoritative',  label: 'Autoritaire & Médical',   emoji: '🩺' },
  { id: 'storytelling',   label: 'Storytelling & Émotionnel', emoji: '📖' },
  { id: 'energetic',      label: 'Énergique & Motivant',    emoji: '⚡' },
  { id: 'conversational', label: 'Convivial & Direct',      emoji: '💬' },
  { id: 'educational',    label: 'Éducatif & Scientifique', emoji: '🎓' },
]

const SS_AUDIENCES = [
  { id: 'seniors',  label: 'Seniors 60+' },
  { id: 'adults',   label: 'Adultes 40–60' },
  { id: 'general',  label: 'Grand public' },
  { id: 'creators', label: 'Créateurs' },
]

const SS_NICHES = [
  'Santé & Bien-être', 'Finance & Argent', 'Développement personnel',
  'Nutrition & Régime', 'Santé mentale', 'Fitness & Sport',
  'Technologie', 'Spiritualité', 'Relations', 'Business',
]

const SS_LENGTHS = [
  { id: 'short',      label: 'Court',         desc: '3–5 min' },
  { id: 'medium',     label: 'Moyen',         desc: '8–12 min' },
  { id: 'long',       label: 'Long',          desc: '15–20 min' },
  { id: 'extra_long', label: 'Très long',     desc: '25–35 min' },
]
...
function NouvelleVideo({ user, onBack, onGoToParams }) {
  ...
  // ── Mode SpeedScript ──
  const [modeGeneration, setModeGeneration] = useState('manuel') // 'manuel' | 'speedscript'
  const [ssTone, setSsTone] = useState('conversational')
  const [ssAudience, setSsAudience] = useState('general')
  const [ssNiche, setSsNiche] = useState('Développement personnel')
  const [ssLongueur, setSsLongueur] = useState('medium')
  ...
  const lancerGeneration = async () => {
    ...
    try {
      const { data: { user: u } } = await supabase.auth.getUser()
      const isSpeedScript = modeGeneration === 'speedscript'
      const res = await fetch('/api/generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: u.id, contenu, duree, avatarId, voiceId, heygenKey,
          titre, description,
          scriptDirect: !isSpeedScript,
          speedscript: isSpeedScript,
          tone: isSpeedScript ? ssTone : undefined,
          audience: isSpeedScript ? ssAudience : undefined,
          niche: isSpeedScript ? ssNiche : undefined,
          longueur: isSpeedScript ? ssLongueur : undefined,
          stopAfterVideo: true,
        })
      })
      const data = await res.json()
        ...
        {etape === 'script' && (
          <div className="space-y-5">
            ...
            <div className="flex gap-2 p-1 bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl">
              {[
                { id: 'manuel', label: 'Script manuel', desc: 'Colle ton script' },
                { id: 'speedscript', label: '⚡ SpeedScript', desc: 'Génération IA avancée' },
              ].map(m => (
                <button key={m.id} onClick={() => setModeGeneration(m.id)}
                  className={`flex-1 py-2.5 px-3 rounded-lg text-left transition-all ${
                    modeGeneration === m.id
                      ? 'bg-[#1a1a1a] border border-[#2a2a2a]'
                      : 'hover:bg-[#111]'
                  }`}>
                  <p className={`text-[12px] font-medium ${modeGeneration === m.id ? 'text-white' : 'text-[#555]'}`}>{m.label}</p>
                  <p className="text-[10px] text-[#333] mt-0.5">{m.desc}</p>
                </button>
              ))}
            </div>
            ...
            {/* Mode SpeedScript */}
            {modeGeneration === 'speedscript' && (
              <div className="space-y-5">

                {/* Contenu source */}
                <Field label="Contenu source" hint="Idée, résumé, article ou script existant à réécrire">
                  <textarea
                    value={contenu}
                    onChange={e => setContenu(e.target.value)}
                    className={`${inputCls} resize-none`}
                    rows={5}
                    placeholder="Décris ton sujet ou colle ton contenu source..."
                  />
                </Field>
                ...
                <Field label="Niche">
                  <div className="flex flex-wrap gap-1.5">
                    {SS_NICHES.map(n => (
                      <button key={n} onClick={() => setSsNiche(n)}
                ...
                <Field label="Ton">
                  <div className="grid grid-cols-1 gap-1.5">
                    {SS_TONES.map(t => (
                      <button key={t.id} onClick={() => setSsTone(t.id)}
                        className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg border text-left transition ${
                          ssTone === t.id
                            ? 'border-[#c0392b] bg-[#c0392b]/8'
                            : 'border-[#1a1a1a] bg-[#0d0d0d] hover:border-[#2a2a2a]'
                        }`}>
                        <span className="text-base leading-none">{t.emoji}</span>
                        <span className={`text-[12px] font-medium ${ssTone === t.id ? 'text-[#c0392b]' : 'text-[#888]'}`}>{t.label}</span>
                        {ssTone === t.id && <div className="ml-auto w-3.5 h-3.5 rounded-full bg-[#c0392b] flex items-center justify-center"><svg width="7" height="7" viewBox="0 0 7 7" fill="none"><path d="M1 3.5L2.5 5 6 1.5" stroke="white" strokeWidth="1.2" strokeLinecap="round"/></svg></div>}
                      </button>
                    ))}
                  </div>
                </Field>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Audience">
                    <div className="space-y-1.5">
                      {SS_AUDIENCES.map(a => (
                        <button key={a.id} onClick={() => setSsAudience(a.id)}
                          className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border text-left transition ${
                            ssAudience === a.id
                              ? 'border-[#c0392b] bg-[#c0392b]/8'
                              : 'border-[#1a1a1a] bg-[#0d0d0d] hover:border-[#2a2a2a]'
                          }`}>
                          <span className={`text-[11px] font-medium ${ssAudience === a.id ? 'text-[#c0392b]' : 'text-[#777]'}`}>{a.label}</span>
                          {ssAudience === a.id && <div className="w-3 h-3 rounded-full bg-[#c0392b]" />}
                        </button>
                      ))}
                    </div>
                  </Field>
                  ...
                  <Field label="Longueur">
                    <div className="space-y-1.5">
                      {SS_LENGTHS.map(l => (
                        <button key={l.id} onClick={() => setSsLongueur(l.id)}
                          className={`w-full flex items-center justify-between px-3 py-2 rounded-lg border text-left transition ${
                            ssLongueur === l.id
                              ? 'border-[#c0392b] bg-[#c0392b]/8'
                              : 'border-[#1a1a1a] bg-[#0d0d0d] hover:border-[#2a2a2a]'
                          ...
                          <div>
                            <p className={`text-[11px] font-medium ${ssLongueur === l.id ? 'text-[#c0392b]' : 'text-[#777]'}`}>{l.label}</p>
                            <p className="text-[10px] text-[#333]">{l.desc}</p>
                          </div>
                          {ssLongueur === l.id && <div className="w-3 h-3 rounded-full bg-[#c0392b]" />}
                        </button>
                      ))}
                    </div>
                  </Field>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="text-[13px] font-semibold text-white">
                    {modeGeneration === 'speedscript' ? 'Choisir l\'avatar' : 'Analyser le script'}
                  </p>
                  <p className="text-[11px] text-white/60">
                    {modeGeneration === 'speedscript' ? 'Le script sera généré automatiquement' : 'Titre · Description · Avatar'}
                  </p>
                </div>
              </div>
            ...
            <div className="border border-[#1a1a1a] rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-[#111]">
                {[
                  ...
                  modeGeneration === 'speedscript'
                    ? { label: 'Génération', value: `⚡ SpeedScript · ${SS_LENGTHS.find(l => l.id === ssLongueur)?.label || ssLongueur}`, icon: <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><polygon points="7.5 1 2 7h5l-1.5 5 6-6.5H7L7.5 1" stroke="currentColor" strokeWidth="1.1" strokeLinejoin="round"/></svg> }
                  ...
                  ...(modeGeneration === 'speedscript' ? [
                    { label: 'Ton', value: SS_TONES.find(t => t.id === ssTone)?.label || ssTone, icon: <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 4h9M2 7h6M2 10h7" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg> },
                    { label: 'Niche', value: ssNiche, icon: <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.2"/><path d="M6.5 3v3.5l2.5 1.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg> },
                  ] : []),
